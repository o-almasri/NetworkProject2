...
Server::Server(PCSTR port, PCSTR AuthserverIp, PCSTR Authport)
{
    this->port = port;
...
bool Server::Initialize()
{
    ...
    //TODO:: try to connect to auth here
//-----------------------------------------------------------------------------------
    std::cout << "Connecting to Auth..." << std::endl;
...
//
    ...
    if (result == SOCKET_ERROR)
    {
        printf("Failed to make connection socket non-blocking! for Auth %d\n", WSAGetLastError());
        closesocket(this->AuthserverSocket);
        WSACleanup();
        return false;
    }
    printf("Connection to Auth successful!\n");

    Client* Auth = new Client();
    Auth->socket = this->AuthserverSocket;
    this->clients.push_back(Auth);
    ...
    while (true)
    {
        ...
        for (int i = this->clients.size() - 1; i >= 0; i--)
        {
            ...
            if (FD_ISSET(client->socket, &readSet))
            {
                ...
                int packetHeader;
                packetHeader = client->buffer.ReadInt();
...
bool Server::SendToAuthServer(char* data, int dataLength) 
{

    int result;
    result = send(this->AuthserverSocket, data, dataLength, 0);
    if (result == SOCKET_ERROR)
    {
        printf("Failed to send message to Auth: %d\n", WSAGetLastError());
        closesocket(this->AuthserverSocket);
        return false;
    }
...
void Server::AddRequest(INT32 RID , Client* client)
{
    
    responseMap[RID] = client;

}



Client* Server::Getclient(INT32 RID)
{
 
    return responseMap[RID];
}