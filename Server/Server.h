...
{
public:
	Server(PCSTR port, PCSTR AuthserverIp, PCSTR Authport);
	~Server();
	...
	// new functions to communicate with auth server
	bool SendToAuthServer(char* data, int dataLength);
	void AddRequest (INT32 RID, Client* CL);
	Client* Getclient(INT32 RID);
	std::map< INT32, Client* > responseMap; // Holds what room a client is in
private:
	void ShutDown();
