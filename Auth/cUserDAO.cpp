...
{
	//std::cout << "Email Update Login " << email << std::endl;
	CreateConnection();
	sql::PreparedStatement* stmt = connection->prepareStatement(
		"UPDATE user SET last_login_date = now() WHERE email = ?;");
	stmt->setString(1, email);
	int result = stmt->executeUpdate();
	ReleaseConnection();
}
...
sUser cUserDAO::FindByEmail(std::string email)
{
	...
	ReleaseConnection();
	return user;
}//id, name, email, age, last_login_date, creation_date


	//get user name by Email ;
std::string cUserDAO::getName(std::string email)
{
	CreateConnection();
	sql::PreparedStatement* stmt = connection->prepareStatement(
	...
	std::string name = resultSet->getString(1);
	ReleaseConnection();
	return name;
